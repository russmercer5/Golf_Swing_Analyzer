# app.py - Complete Golf Swing Analysis WebApp
import cv2
...
app = Flask(__name__)
app.secret_key = 'golf_swing_analysis_2024_secret_key'
...
# Color scheme for metrics display
METRIC_COLORS = {
    'shoulder_turn': '#4fc3f7',
    'hip_turn': '#66bb6a',
    'shoulder_tilt': '#ffa726',
    'hip_tilt': '#ef5350',
    'shoulder_sway': '#ab47bc',
    'hip_sway': '#26c6da'
}
...
class GolfSwingAnalyzer:
    ...
    def calculate_metrics(self, landmarks, frame_shape, view_type='down_the_line'):
        ...
        left_elbow = self.get_landmark(landmarks, mp_pose.PoseLandmark.LEFT_ELBOW)
        right_elbow = self.get_landmark(landmarks, mp_pose.PoseLandmark.RIGHT_ELBOW)
        left_wrist = self.get_landmark(landmarks, mp_pose.PoseLandmark.LEFT_WRIST)
        right_wrist = self.get_landmark(landmarks, mp_pose.PoseLandmark.RIGHT_WRIST)
    ...
    def detect_swing_positions(self, landmarks_history, fps):
        ...
        # Simplified detection - in production, use ML or more sophisticated heuristics
        # Track key landmarks to detect swing phases
        for i, landmarks in enumerate(landmarks_history):
            ...
            if None in [left_shoulder, right_shoulder, right_wrist]:
                continue
            
            # Calculate key angles for position detection
            # Club position relative to body (simplified)
            wrist_to_shoulder_y = right_wrist.y - right_shoulder.y
            wrist_to_shoulder_x = right_wrist.x - right_shoulder.x
            
            # Detect positions based on wrist position relative to body
            # Position 1: Setup (arms hanging down)
            if i < len(landmarks_history) * 0.1:
                positions[1] = i
                position_frames[1] = landmarks
            
            # Position 4: Top of backswing (wrist highest)
            elif i > len(landmarks_history) * 0.25 and i < len(landmarks_history) * 0.4:
                if right_wrist.y < right_shoulder.y and right_wrist.y < left_shoulder.y:
                    positions[4] = i
                    position_frames[4] = landmarks
            
            # Position 7: Impact (wrist near body, elbows bent)
            elif i > len(landmarks_history) * 0.45 and i < len(landmarks_history) * 0.6:
                positions[7] = i
                position_frames[7] = landmarks
            
            # Position 9: Finish (arms high, body rotated)
            elif i > len(landmarks_history) * 0.8:
                positions[9] = i
        ...
        for pos in range(1, 10):
            if pos not in positions:
                # Use interpolation for missing positions
                frame_idx = int((pos - 1) * len(landmarks_history) / 8)
                if 0 <= frame_idx < len(landmarks_history):
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-pdf { background: #ef5350; color: #fff; }
